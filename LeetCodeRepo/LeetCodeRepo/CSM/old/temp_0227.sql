...
SELECT sb.ResourceGUID,sh.WorkloadName,sh.ServiceName,'TopCustomer' AS Source, GETDATE() AS ProcessedDate    ---- COALESCE(sub.TPID,hist.TPID,hist1.TPID) AS TPID, sub.SubscriptionGUID INTO #tmpTPID 
...
LEFT OUTER JOIN vwServiceBilling sb on sb.AI_SubscriptionKey = sub.SubscriptionGUID   --vwUsageMonthly
LEFT OUTER JOIN [Bigcat].[vwServiceHierarchy_V2] sh ON sb.ResourceGUID = sh.ConsumptionResourceGUID
where sb.billingmonth = '20200201'